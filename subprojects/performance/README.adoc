...
For detailed allocation profiling with GC metrics:
    ...
    ./gradlew -Pindy=true -PbenchInclude=MemoryAllocation :performance:jmh -Pjmh.profilers=gc
...
The comparison report is saved to `build/results/jmh-compare/comparison-report.txt`.
...
Results saved to `build/results/jmh-compare/threshold-sweep/threshold-summary.txt`.
...
[source,bash]
----
./gradlew -Pindy=true -PbenchInclude="ColdCall|RequestLifecycle|Template" :performance:jmh
----
...
[source,bash]
----
./gradlew -Pindy=true -PbenchInclude="CacheInvalidation|Dispatch" :performance:jmh
----
